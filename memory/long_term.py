...
from models.document import JdChunk, JdDocument
...
def _load_bm25_index() -> tuple[SimpleBM25, dict[int, int]]:
    ...
    idx_to_chunk_id = {i: r[0] for i, r in enumerate(rows)}  # list下标 → DB自增ID
    bm25 = SimpleBM25()
    bm25.fit(docs)
    logger.info(f"BM25 索引构建完成: {len(docs)} 个文档, ID 范围 {min(idx_to_chunk_id.values()) if idx_to_chunk_id else 0}-{max(idx_to_chunk_id.values()) if idx_to_chunk_id else 0}")
    return bm25, idx_to_chunk_id


def clear_bm25_cache():
    """新 JD 入库后清除 BM25 索引缓存，下次检索时重建"""
...
def _rrf_merge(
    ...
    for chunk_id, score in merged:
        if chunk_id in chunk_id_to_item:
            ...
            # 仅向量结果有语义分数，BM25-only 不设 cos_sim
