...
6. **不要做硬性技能扣分**：缺少个别技能不应直接判 weak，要看整体匹配度。
7. 对初级/实习岗位：允许以项目经验、学习能力、迁移能力弥补经验不足。
8. 对非技术岗位（产品/数据/方案）：关注产品能力、数据分析、业务理解、沟通协作等综合素质，不能只看技术技能覆盖率。
9. 关键风险（critical gap）才显著降低整体等级，普通学习缺口进入 learning_plan。
10. strengths 要具体描述候选人做得好的地方，gaps 要指出具体的差距，learning_plan 要给出可执行的学习建议，不要泛泛而谈。
